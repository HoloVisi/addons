...
class networkMount(object):
    ...
    def __init__(self, mount_id, holoMain):
        try:

            holoMain.dbg_log('networkMount::__init__', 'enter_function',
                           0)
            ...
            self.holo = holoMain
            self.winHoloMount = holoWindows.mainWindow('mainWindow.xml',
                    self.holo.__cwd__, 'Default', holoMain=holoMain,
                    isChild=True)
            self.mount_id = mount_id
            self.holo.dictModules['networkMount'] = self
            self.winHoloMount.show()
            self.current_mountpoint = None

            if self.mount_id != 'new_mount':

                mount_dict = self.holo.read_node('mounts')
            ...
            for strEntry in sorted(self.struct, key=lambda x: \
                ...
                dictProperties = {
                    'modul': 'networkMount',
                    'listTyp': self.holo.listObject['list'],
                    'menuLoader': 'menu_loader',
                    'category': strEntry,
                    }

                self.winHoloMount.addMenuItem(self.struct[strEntry]['name'
                        ], dictProperties)

            self.holo.set_busy(0)

            self.winHoloMount.doModal()

            del self.winHoloMount
            del self.holo.dictModules['networkMount']

            holoMain.dbg_log('networkMount::__init__', 'exit_function', 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('networkMount::__init__', 'ERROR: ('
                            + repr(e) + ')', 4)

    def menu_loader(self, menuItem):
        try:

            self.holo.dbg_log('networkMount::menu_loader',
                            'enter_function', 0)

            self.winHoloMount.showButton(3, 32140, 'networkMount',
                    'save_mount')

            if self.mount_id != 'new_mount':
                self.winHoloMount.showButton(2, 32141, 'networkMount',
                        'delete_mount')
            category = 'mount'
            for entry in sorted(self.struct[category]['settings'],
                ...
                if not 'parent' in self.struct[category]['settings'
                    ...
                    self.winHoloMount.addConfigItem(self.holo._(self.struct[category]['settings'
                            ][entry]['name']), dictProperties,
                            self.holo.listObject['list'])
                else:
            ...
            self.holo.dbg_log('networkMount::menu_loader', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.dbg_log('networkMount::menu_loader', 'ERROR: ('
                            + repr(e) + ')', 4)

    def set_value(self, listItem):
        try:

            self.holo.dbg_log('networkMount::set_value', 'enter_function'
                            , 0)
            ...
            self.holo.dbg_log('networkMount::set_value', 'exit_function',
                            0)
        except Exception, e:

            self.holo.dbg_log('networkMount::set_value', 'ERROR: ('
                            + repr(e) + ')', 4)

    def save_mount(self):
        try:

            self.holo.dbg_log('save_mount::save_mount', 'enter_function',
                            0)

            if self.struct['mount']['settings']['mountpoint']['value'] \
                == '':
                dialog = xbmcgui.Dialog()
                dialog.ok('Mount', self.holo._(32361), '',
                          self.holo._(32352))
                return
            ...
            if self.current_mountpoint != None \
                ...
                umount = self.holo.execute('umount /media/'
            ...
            if self.mount_id == 'new_mount':
                ...
                self.holo.remove_node(self.mount_id)
            ...
            self.holo.write_setting(mount_uuid, 'type',
                                  self.struct['mount']['settings'
                                  ]['type']['value'], 'mounts')

            mount_info['mountpoint'] = self.struct['mount']['settings'
                    ]['mountpoint']['value']
            self.holo.write_setting(mount_uuid, 'mountpoint',
                                  self.struct['mount']['settings'
                                  ]['mountpoint']['value'], 'mounts')

            mount_info['server'] = self.struct['mount']['settings'
                    ]['server']['value']
            self.holo.write_setting(mount_uuid, 'server',
                                  self.struct['mount']['settings'
                                  ]['server']['value'], 'mounts')

            mount_info['share'] = self.struct['mount']['settings'
                    ]['share']['value']
            self.holo.write_setting(mount_uuid, 'share',
                                  self.struct['mount']['settings'
                                  ]['share']['value'], 'mounts')

            mount_info['user'] = self.struct['mount']['settings']['user'
                    ]['value']
            self.holo.write_setting(mount_uuid, 'user',
                                  self.struct['mount']['settings'
                                  ]['user']['value'], 'mounts')

            mount_info['pass'] = self.struct['mount']['settings']['pass'
                    ]['value']
            self.holo.write_setting(mount_uuid, 'pass',
                                  self.struct['mount']['settings'
                                  ]['pass']['value'], 'mounts')

            mount_info['options'] = self.struct['mount']['settings'
                    ]['options']['value']
            self.holo.write_setting(mount_uuid, 'options',
                                  self.struct['mount']['settings'
                                  ]['options']['value'], 'mounts')

            self.holo.dictModules['connman'].mount_drive(mount_info)

            self.holo.dbg_log('save_mount::save_mount', 'exit_function',
                            0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('save_mount::save_mount', 'ERROR: ('
                            + repr(e) + ')', 4)

    def delete_mount(self):
        try:

            self.holo.dbg_log('save_mount::delete_mount', 'enter_function'
                            , 0)
            ...
            self.holo.dbg_log('save_mount::delete_mount', 'exit_function'
                            , 0)

            return 'close'
        except Exception, e:

            self.holo.dbg_log('save_mount::delete_mount', 'ERROR: ('
                            + repr(e) + ')', 4)
...
class connmanService(object):
    ...
    def __init__(self, servicePath, holoMain):
        try:

            holoMain.dbg_log('connmanService::__init__', 'enter_function', 0)
            ...
            self.winHoloCon = holoWindows.mainWindow('mainWindow.xml',
                    self.holo.__cwd__, 'Default', holoMain=holoMain,
                    isChild=True)
            ...
            self.holo.dictModules['connmanNetworkConfig'] = self
            ...
            for strEntry in sorted(self.struct, key=lambda x: \
                ...
                self.winHoloCon.addMenuItem(self.struct[strEntry]['name'
                        ], dictProperties)

            self.holo.set_busy(0)

            self.winHoloCon.doModal()

            del self.winHoloCon
            del self.holo.dictModules['connmanNetworkConfig']

            self.holo.dbg_log('connmanService::__init__', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connmanService::__init__', 'ERROR: ('
                            + repr(e) + ')', 4)

    def cancel(self):
        try:

            self.holo.dbg_log('connmanService::cancel', 'exit_function',
                            0)

            self.holo.set_busy(1)
            self.winHoloCon.close()
            self.holo.set_busy(0)
            self.holo.dbg_log('connmanService::cancel', 'exit_function',
                            0)
        except Exception, e:
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connmanService::cancel', 'ERROR: ('
                            + repr(e) + ')', 4)

    def menu_loader(self, menuItem):
        try:

            self.holo.dbg_log('connmanService::menu_loader',
                            'enter_function', 0)

            self.winHoloCon.showButton(3, 32140, 'connmanNetworkConfig',
                    'save_network')

            self.winHoloCon.showButton(2, 32212, 'connmanNetworkConfig',
                    'cancel')

            if self.servicePath not in ['new_vpn']:
                if self.service_properties['Type'] in ['wifi']:
                    if self.service_properties['Favorite'] == True:
                        self.winHoloCon.showButton(1, 32141,
                                'connmanNetworkConfig', 'delete_network'
                                )

                if self.service_properties['State'] in ['ready',
                        'online']:
                    self.winHoloCon.showButton(
                        4,
                        32143,
            ...
            for entry in sorted(self.struct[category]['settings'],
                ...
                if not 'parent' in self.struct[category]['settings'
                    ...
                    self.winHoloCon.addConfigItem(self.holo._(self.struct[category]['settings'
                            ][entry]['name']), dictProperties,
            ...
            self.holo.dbg_log('connmanService::menu_loader', 'ERROR: ('
                            + repr(e) + ')', 4)

    def set_value(self, listItem):
        try:

            self.holo.dbg_log('connmanService::set_value',
                            'enter_function', 0)
            ...
            self.holo.dbg_log('connmanService::set_value', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.dbg_log('connmanService::set_value', 'ERROR: ('
                            + repr(e) + ')', 4)
    ...
    def dbus_config(self, category):
        try:
            ...
            self.holo.dbg_log('connmanService::dbus_config', 'ERROR: ('
                            + repr(e) + ')', 4)
    ...
    def save_network(self):
        try:
            ...
            self.holo.dbg_log('connmanService::save_network',
                            'enter_function', 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connmanService::save_network', 'ERROR: ('
                            + repr(e) + ')', 4)
            return 'close'
    ...
    def delete_network(self):
        try:
            ...
            self.holo.dbg_log('connmanService::delete_network',
                            'enter_function', 0)

            self.holo.dictModules['connman'].delete_network(None)
            ...
            self.holo.dbg_log('connmanService::delete_network', 'ERROR: ('
                             + repr(e) + ')', 4)
            return 'close'
    ...
    def connect_network(self):
        try:
            ...
            self.holo.dbg_log('connmanService::connect_network',
                            'enter_function', 0)

            self.holo.dictModules['connman'].connect_network(None)
    ...
    def disconnect_network(self):
        try:
            ...
            self.holo.dbg_log('connmanService::disconnect_network',
                            'enter_function', 0)

            self.holo.dictModules['connman'].disconnect_network(None)
...
class connmanVpn(object):
    ...
    def __init__(self, vpn, holoMain):
        try:

            holoMain.dbg_log('connmanVpn::__init__', 'enter_function', 0)
            ...
            self.holo.dictModules['connmanVpnConfig'] = self
            ...
            self.winHoloCon.addMenuItem(self.struct['Provider']['name'],
                    dictProperties)
            ...
            del self.winHoloCon
            del self.holo.dictModules['connmanVpnConfig']

            self.holo.dbg_log('connmanVpn::__init__', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connmanVpn::__init__', 'ERROR: ('
                            + repr(e) + ')', 4)

    def cancel(self):
        try:

            self.holo.dbg_log('connmanVpn::cancel', 'exit_function', 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connmanVpn::cancel', 'ERROR: (' + repr(e)
                            + ')', 4)

    def show_advanced(self, listItem=None):
        try:

            self.holo.dbg_log('connmanVpn::show_advanced', 'exit_function'
                            , 0)
            ...
            self.holo.dbg_log('connmanVpn::show_advanced', 'ERROR: ('
                            + repr(e) + ')', 4)

    def menu_loader(self, menuItem):
        try:

            self.holo.dbg_log('connmanVpn::menu_loader', 'enter_function'
                            , 0)

            self.winHoloCon.showButton(3, 32140, 'connmanVpnConfig',
                    'save_vpn_config')

            self.winHoloCon.showButton(2, 32212, 'connmanVpnConfig',
                    'cancel')

            if self.vpn_name not in ['__new__']:
                self.winHoloCon.showButton(1, 32141, 'connmanVpnConfig',
                        'delete_vpn_config')
            ...
            if self.struct[category]['settings']['Type']['value'] != '':
                ...
                self.winHoloCon.addConfigItem('Advanced',
                        {'typ': 'separator'},
                        menuItem.getProperty('listTyp'))
                ...
                self.winHoloCon.addConfigItem('Show Advanced',
            ...
            self.holo.dbg_log('connmanVpn::menu_loader', 'exit_function',
                            0)
        except Exception, e:

            self.holo.dbg_log('connmanVpn::menu_loader', 'ERROR: ('
                            + repr(e) + ')', 4)

    def set_value(self, listItem):
        try:

            self.holo.dbg_log('connmanVpn::set_value', 'enter_function',
                            0)
            ...
            self.holo.dbg_log('connmanVpn::set_value', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connmanVpn::set_value', 'ERROR: ('
                            + repr(e) + ')', 4)
    ...
    def load_vpn_config(self, vpn_name):
        ...
        try:
            ...
            self.holo.dbg_log('connmanVpn::load_vpn_config',
                            'enter_function', 0)
            ...
            self.holo.dbg_log('connmanVpn::load_vpn_config', 'ERROR: ('
                            + repr(e) + ')', 4)

    def save_vpn_config(self):
        try:

            self.holo.dbg_log('connmanVpn::save_vpn_config',
                            'enter_function', 0)
            ...
            for entry in sorted(self.struct['Provider']['settings'],
                ...
                if 'parent' in self.struct['Provider']['settings'
                    ...
                    if self.struct['Provider']['settings']['Type'
                        ...
                        dialog = xbmcgui.Dialog()
                        self.holo.set_busy(0)
                        dialog.ok('HoloVisi VPN', self.holo._(32378))
                        self.holo.dbg_log('connmanVpn::save_vpn_config',
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connmanVpn::save_vpn_config', 'ERROR: ('
                            + repr(e) + ')', 4)

    def delete_vpn_config(self):
        try:

            self.holo.dbg_log('connmanVpn::delete_vpn_config',
                            'enter_function', 0)
...
class connman:
    ...
    def __init__(self, holoMain):
        try:
            ...
            holoMain.dbg_log('connman::__init__', 'enter_function', 0)
            ...
            self.wait_conf_file = \
                '/storage/.cache/holovisi/network_wait'

            self.busy = 0
            self.holo = holoMain
            self.holo.dbg_log('connman::__init__', 'exit_function', 0)
            self.vpn_conf_dir = '/storage/.config/vpn-config/'
        except Exception, e:

            self.holo.dbg_log('connman::__init__', 'ERROR: (' + repr(e)
                            + ')', 4)
    ...
    def clear_list(self):
        try:
            ...
            self.holo.dbg_log('connman::clear_list', 'ERROR: (' + repr(e)
                            + ')', 4)

    def do_init(self):
        try:

            self.holo.dbg_log('connman::do_init', 'enter_function', 0)
            ...
            self.dbusMonitor = monitorLoop(self.holo, self.dbusSystemBus)
            self.dbusMonitor.start()

            self.holo.dbg_log('connman::do_init', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::do_init', 'ERROR: (' + repr(e)
                            + ')', 4)

    def exit(self):
        try:

            self.holo.dbg_log('connman::exit', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::exit', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::exit', 'ERROR: (' + repr(e) + ')'
                            , 4)

    def load_values(self):
        try:

            self.holo.dbg_log('connman::load_values', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::load_values', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::load_values', 'ERROR: ('
                            + repr(e) + ')')
    ...
    def menu_connections(
        ...
        try:
            ...
            self.holo.dbg_log('connman::menu_connections',
                            'enter_function', 0)

            self.holo.set_busy(1)
            self.holo.dbg_log('connman::menu_connections__busy__',
                            str(self.holo.__busy__), 0)
            ...
            if len(self.dbusServices) != len(self.listItems):
                rebuildList = 1
                self.holo.winHoloMain.getControl(int(self.holo.listObject['netlist'
                        ])).reset()
                self.clear_list()
            else:
                ...
                if rebuildList == 1:
                    self.listItems[dbusServicePath] = \
                        self.holo.winHoloMain.addConfigItem(apName,
                            dictProperties, self.holo.listObject['netlist'
                            ])

            self.holo.set_busy(0)

            self.holo.dbg_log('connman::menu_connections', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connman::menu_connections', 'ERROR: ('
                            + repr(e) + ')', 4)

    def menu_loader(self, menuItem=None):
        try:

            self.holo.dbg_log('connman::menu_loader', 'enter_function0',
                            0)
            ...
            if menuItem == None:
                menuItem = \
                    self.holo.winHoloMain.getControl(self.holo.winHoloMain.guiMenList).getSelectedItem()
            ...
            for (path, technologie) in self.technologie_properties:
                ...
                if path in self.struct:
                    self.holo.winHoloMain.addConfigItem(self.holo._(self.struct[path]['name'
                            ]), {'typ': 'separator'},
                            menuItem.getProperty('listTyp'))
                    ...
                    for entry in sorted(self.struct[path]['settings'],
                        ...
                        if 'InfoText' in self.struct[path]['settings'
                            ...
                            dictProperties['InfoText'] = \
                                self.holo._(self.struct[path]['settings'
                                    ][entry]['InfoText'])
                        ...
                        if not 'parent' in self.struct[path]['settings'
                            ...
                            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct[path]['settings'
                                    ][entry]['name']), dictProperties,
                                    menuItem.getProperty('listTyp'))
                        else:
            ...
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['vpn'
                    ]['name']), {'typ': 'separator'},
                    menuItem.getProperty('listTyp'))
            ...
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['vpn'
                    ...
                    ]['action'], 'InfoText': self.holo._(self.struct['vpn'
                    ]['settings']['add']['InfoText'])},
                    menuItem.getProperty('listTyp'))

            # Timeservers
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['Timeservers'
                    ]['name']), {'typ': 'separator'},
                    menuItem.getProperty('listTyp'))
            ...
            if 'Timeservers' in self.clock_properties:
                for setting in sorted(self.struct['Timeservers'
                    ...
                    if 'InfoText' in self.struct['Timeservers'
                        ...
                        dictProperties['InfoText'] = \
                            self.holo._(self.struct['Timeservers'
            ...
            # Mounts
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['mounts'
                    ]['name']), {'typ': 'separator'},
                    menuItem.getProperty('listTyp'))
            ...
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['mounts'
                ...
                'entry': 'new_mount',
                'InfoText': self.holo._(self.struct['mounts']['settings'
                        ]['add']['InfoText']),
                }, menuItem.getProperty('listTyp'))
            ...
            if 'mounts' in mount_dict:
                for mount in mount_dict['mounts']:
                    ...
                    self.holo.winHoloMain.addConfigItem(mount_dict['mounts'
                            ][mount]['mountpoint'], dictProperties,
                            menuItem.getProperty('listTyp'))

            # Network Wait
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['advanced'
                    ]['name']), {'typ': 'separator'},
                    menuItem.getProperty('listTyp'))
            ...
            self.holo.winHoloMain.addConfigItem(self.holo._(self.struct['advanced'
                ...
                'InfoText': self.holo._(self.struct['advanced']['settings'
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::menu_loader', 'exit_function', 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connman::menu_loader', 'ERROR: ('
                            + repr(e) + ')', 4)

    def open_context_menu(self, listItem):
        try:

            self.holo.dbg_log('connman::open_context_menu',
                            'enter_function', 0)
            ...
            if listItem is None:
                listItem = \
                    self.holo.winHoloMain.getControl(self.holo.listObject['netlist'
                        ]).getSelectedItem()
            ...
            if listItem.getProperty('Favorite') == '1' \
                ...
                values[2] = {'text': self.holo._(32150),
            ...
            self.holo.dbg_log('connman::open_context_menu', 'ERROR: ('
                            + repr(e) + ')', 4)
    ...
    def set_timeservers(self, **kwargs):
        try:
            ...
            self.holo.dbg_log('connman::set_timeservers', 'enter_function'
                            , 0)
            ...
            self.holo.dbg_log('connman::set_timeservers', 'exit_function'
                            , 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::set_timeservers', 'ERROR: ('
                            + repr(e) + ')', 4)

    def set_value(self, listItem=None):
        try:

            self.holo.dbg_log('connman::set_value', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::set_value', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::set_value', 'ERROR: (' + repr(e)
                            + ')', 4)

    def set_technologie(self, **kwargs):
        try:

            self.holo.dbg_log('connman::set_technologies',
                            'enter_function', 0)
                ...
                if path == techPath:
                    for setting in self.struct[techPath]['settings']:
                        ...
                        if settings['Powered']['value'] == '1':
                            ...
                            if not technologie['Powered'] is False:
                                self.Technology.SetProperty('Powered',
                                        dbus.Boolean(False, variant_level=1))
            ...
            self.holo.dbg_log('connman::set_technologies', 'exit_function', 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connman::set_technologies', 'ERROR: ('
                            + repr(e) + ')', 4)

    def configure_network(self, listItem=None):
        try:

            self.holo.dbg_log('connman::configure_network',
                            'enter_function', 0)
            ...
            if not listItem.getProperty('netType') == 'vpn':
                ...
                pid = self.holo.execute('pidof %s' % 'connman-vpnd'
                        ).split(' ')
                if len(pid) > 0:
            ...
            self.holo.dbg_log('connman::configure_network', 'ERROR: ('
                            + repr(e) + ')', 4)

    def connect_network(self, listItem=None):
        try:

            self.holo.dbg_log('connman::connect_network', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::connect_network', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connman::connect_network', 'ERROR: ('
                            + repr(e) + ')', 4)

    def connect_reply_handler(self):
        try:

            self.holo.dbg_log('connman::connect_reply_handler',
                            'enter_function', 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::connect_reply_handler', 'ERROR: ('
                             + repr(e) + ')', 4)

    def dbus_error_handler(self, error):
        try:

            self.holo.dbg_log('connman::dbus_error_handler',
                            'enter_function', 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::dbus_error_handler', 'ERROR: ('
                            + repr(e) + ')', 4)

    def disconnect_network(self, listItem=None):
        try:

            self.holo.dbg_log('connman::disconnect_network',
                            'enter_function', 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::disconnect_network', 'ERROR: ('
                            + repr(e) + ')', 4)

    def delete_network(self, listItem=None):
        try:

            self.holo.dbg_log('connman::delete_network', 'enter_function'
                            , 0)
            ...
            self.holo.dbg_log('connman::delete_network', 'exit_function',
                            0)
        except Exception, e:

            self.holo.set_busy(0)
            self.holo.dbg_log('connman::delete_network', 'ERROR: (' + repr(e) + ')', 4)

    def refresh_network(self, listItem=None):
        try:

            self.holo.dbg_log('connman::refresh_network', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::refresh_network', 'exit_function', 0)

        except Exception, e:
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::refresh_network', 'ERROR: (' + repr(e) + ')', 4)

    def add_vpn(self, listItem=None):
        try:

            self.holo.dbg_log('connman::add_vpn', 'enter_function', 0)

            self.configure_vpn = connmanVpn('__new__', self.holo)
            ...
            self.holo.dbg_log('connman::add_vpn', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::add_vpn', 'ERROR: (' + repr(e)
                            + ')', 4)

    def get_service_path(self):
        try:

            self.holo.dbg_log('connman::get_service_path',
                            'enter_function', 0)

            if hasattr(self, 'winHoloCon'):
                return self.winHoloCon.service_path
            else:
                listItem = \
                    self.holo.winHoloMain.getControl(self.holo.listObject['netlist'
                        ]).getSelectedItem()
                self.holo.dbg_log('connman::get_service_path', 'exit_function' , 0)
                return listItem.getProperty('entry')

        except Exception, e:
            self.holo.dbg_log('connman::get_service_path', 'ERROR: (' + repr(e) + ')', 4)

    def edit_mount(self, listItem=None):
        try:

            self.holo.dbg_log('connman::add_mount', 'enter_function', 0)

            self.configureMount = \
                networkMount(listItem.getProperty('entry'), self.holo)
            del self.configureMount

            self.holo.dbg_log('connman::add_mount', 'enter_function', 0)
        
        except Exception, e:
            self.holo.dbg_log('connman::add_mount', 'ERROR: (' + repr(e) + ')')

    def mount_drives(self):
        try:

            self.holo.dbg_log('connman::mount_drives', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::mount_drives', 'exit_function', 0)

        except Exception, e:
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::mount_drives', 'ERROR: (' + repr(e) + ')')
    ...
    def mount_drive(self, mount_info):
        try:
            ...
            self.holo.dbg_log('connman::mount_drive', 'enter_function', 0)
            ...
            if mount_info['type'] == 'cifs':
                ...
                self.holo.dbg_log('connman::umount_drive',
                                self.holo.execute('umount '
                ...
                self.holo.dbg_log('connman::mount_drive', mount_command
                                + '"', 0)
                ...
                self.holo.dbg_log('connman::mount_drive',
                                self.holo.execute(mount_command), 0)
            ...
            self.holo.set_busy(0)
            self.holo.dbg_log('connman::mount_drive', 'ERROR: ('
                            + repr(e) + ')')

    def start_service(self):
        try:

            self.holo.dbg_log('connman::start_service', 'enter_function',
                            0)

            self.mount_drives()

            self.holo.dbg_log('connman::start_service', 'exit_function',
                            0)
        except Exception, e:

            self.holo.dbg_log('connman::start_service', 'ERROR: ('
                            + repr(e) + ')')

    def stop_service(self):
        try:

            self.holo.dbg_log('connman::stop_service', 'enter_function',
                            0)

            self.holo.dbg_log('connman::stop_service', 'exit_function', 0)
        except Exception, e:
    ...
    def set_network_wait(self, **kwargs):
        try:
            ...
            self.holo.dbg_log('connman::set_network_wait',
                            'enter_function', 0)
            ...
            self.holo.dbg_log('connman::set_network_wait', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.dbg_log('system::set_network_wait', 'ERROR: ('
                            + repr(e) + ')')

    def do_wizard(self):
        try:

            self.holo.dbg_log('connman::do_wizard', 'enter_function', 0)

            self.holo.winHoloMain.set_wizard_title(self.holo._(32305))
            self.holo.winHoloMain.set_wizard_text(self.holo._(32306))
            self.holo.winHoloMain.set_wizard_button_title('')
            self.holo.winHoloMain.set_wizard_list_title(self.holo._(32309))

            self.holo.winHoloMain.getControl(1391).setLabel('show')

            self.holo.winHoloMain.getControl(self.holo.winHoloMain.buttons[1]['id'
                    ]).controlUp(self.holo.winHoloMain.getControl(self.holo.winHoloMain.guiNetList))
            self.holo.winHoloMain.getControl(self.holo.winHoloMain.buttons[1]['id'
                    ]).controlLeft(self.holo.winHoloMain.getControl(self.holo.winHoloMain.guiNetList))

            self.menu_connections(None)

            self.holo.dbg_log('connman::do_wizard', 'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::do_wizard', 'ERROR: (' + repr(e)
                            + ')')
...
class monitorLoop(threading.Thread):
    ...
    def __init__(self, holoMain, dbusSystemBus):
        try:

            holoMain.dbg_log('connman::monitorLoop::__init__',
                           'enter_function', 0)
            ...
            self.wifiAgentPath = '/HoloVisi/agent_wifi'
            self.vpnAgentPath = '/HoloVisi/agent_vpn'

            threading.Thread.__init__(self)

            self.holo.dbg_log('connman::monitorLoop::__init__',
                            'exit_function', 0)
        except Exception, e:

            self.holo.dbg_log('connman::monitorLoop::__init__', 'ERROR: ('
                             + repr(e) + ')', 4)

    def run(self):
        try:

            self.holo.dbg_log('connman::monitorLoop::run',
                            'enter_function', 0)
            ...
            try:
                self.holo.dbg_log('Connman Monitor started.', '', 1)
                self.mainLoop.run()
                self.holo.dbg_log('Connman Monitor stopped.', '', 1)
            except:
                pass

            self.holo.dbg_log('connman::monitorLoop::run', 'exit_function'
                            , 0)
        except Exception, e:

            self.holo.dbg_log('connman::monitorLoop::run', 'ERROR: ('
                            + repr(e) + ')', 4)

    def exit(self):
        try:

            self.holo.dbg_log('connman::monitorLoop::exit',
                            'enter_function', 0)
            ...
            try:
                ...
                if hasattr(self, 'wifiAgent'):
                    ...
                    self.holo.dbg_log('connman::agentLoop::UnregisterAgent'
                                    , '(WIFI)', 0)

                    self.dbusConnmanManager = None
            except Exception, e:
                self.holo.dbg_log('connman::agentLoop::UnregisterAgent (wifi)'
                                , 'ERROR: (' + repr(e) + ')', 4)
            ...
            try:
                ...
                self.holo.dbg_log('connman::agentLoop::UnregisterAgent (vpn)'
                                , 'ERROR: (' + repr(e) + ')', 4)
            ...
            self.holo.dbg_log('connman::monitorLoop::quit', 'ERROR: ('
                            + repr(e) + ')', 4)

    def nameOwnerChanged(self, proxy):
        try:

            self.holo.dbg_log('connman::agentLoop::nameOwnerChanged',
                            'enter_function', 0)
            ...
            self.holo.dbg_log('connman::agentLoop::nameOwnerChanged', 'ERROR: (' + repr(e) + ')', 4)

    def vpnNameOwnerChanged(self, proxy):
        try:

            self.holo.dbg_log('connman::agentLoop::vpnNameOwnerChanged',
                            'enter_function', 0)
    ...
    def managerPropertyChanged(
        ...
        try:
            ...
            self.holo.dbg_log('connman::monitorLoop::managerPropertyChanged'
                            , 'enter_function', 0)
    ...
    def propertyChanged(
        ...
        try:
            ...
            self.holo.dbg_log('connman::monitorLoop::propertyChanged',
                            'enter_function', 0)
    ...
    def technologyChanged(
        ...
        try:
            ...
            self.holo.dbg_log('connman::monitorLoop::technologyChanged',
                            'enter_function', 0)
    ...
    def servicesChanged(self, services, removed):
        try:
            ...
            self.holo.dbg_log('connman::monitorLoop::servicesChanged',
                            'enter_function', 0)
    ...
    def updateList(self, services={}, removed={}):
        try:
            ...
            self.holo.dbg_log('connman::monitorLoop::updateList',
                            'enter_function', 0)

            self.holo.dictModules['connman'].menu_connections(None,
    ...
    def updateGui(
        ...
        try:
            ...
            if not path in self.holo.dictModules['connman'].listItems \
                or self.holo.dictModules['connman'].listItems[path] \
                == None:
                return

            self.holo.dbg_log('connman::monitorLoop::updateGui',
                            'enter_function', 0)

            if name == 'Strength':
                value = str(int(value))
                self.holo.dictModules['connman'
            ...
            self.holo.dbg_log('connman::monitorLoop::updateGui', 'exit_function (KeyError)', 0)
            self.updateList()
            
        except Exception, e:
            self.holo.dbg_log('connman::monitorLoop::updateGui', 'ERROR: (' + repr(e) + ')', 4)

    def forceRender(self):
        try:

            self.holo.dbg_log('connman::monitorLoop::forceRender', 'enter_function', 0)

            focusId = self.holo.winHoloMain.getFocusId()
            self.holo.winHoloMain.setFocusId(self.holo.listObject['netlist'])
            self.holo.winHoloMain.setFocusId(focusId)

            self.holo.dbg_log('connman::monitorLoop::forceRender', 'exit_function', 0)
            
        except Exception, e:
            self.holo.dbg_log('connman::monitorLoop::forceRender', 'ERROR: (' + repr(e) + ')', 4)
...
class wifiAgent(dbus.service.Object):
    ...
    def RequestInput(self, path, fields):
        ...
        try:
            ...
            self.holo.dbg_log('connman::wifiAgent::RequestInput',
                            'enter_function', 0)
            ...
            if fields.has_key('Name'):
                xbmcKeyboard = xbmc.Keyboard('', self.holo._(32146))
            ...
            if fields.has_key('Passphrase'):
                xbmcKeyboard = xbmc.Keyboard('', self.holo._(32147))
                xbmcKeyboard.doModal()
                if xbmcKeyboard.isConfirmed():
                    if xbmcKeyboard.getText() != '':
            ...
            if fields.has_key('Username'):
                xbmcKeyboard = xbmc.Keyboard('', self.holo._(32148))
                xbmcKeyboard.doModal()
                if xbmcKeyboard.isConfirmed():
                    if xbmcKeyboard.getText() != '':
            ...
            if fields.has_key('Password'):
                xbmcKeyboard = xbmc.Keyboard('', self.holo._(32148), True)
                xbmcKeyboard.doModal()
                if xbmcKeyboard.isConfirmed():
                    if xbmcKeyboard.getText() != '':
            ...
            self.holo.dbg_log('connman::monitorLoop::RequestInput',
                            'ERROR: (' + repr(e) + ')', 4)

    @dbus.service.method('net.connman.Agent', in_signature='os',
                         out_signature='')
    def RequestBrowser(self, path, url):
        self.holo.dbg_log('connman::wifiAgent::RequestBrowser',
                        'enter_function', 0)
        self.holo.dbg_log('connman::wifiAgent::RequestBrowser',
                        'exit_function', 0)
        return
    ...
    def ReportError(self, path, error):
        self.holo.dbg_log('connman::wifiAgent::ReportError',
                        'enter_function', 0)
        self.holo.dbg_log('connman::wifiAgent::ReportError',
    ...
    @dbus.service.method('net.connman.Agent', in_signature='',
                         out_signature='')
    def Cancel(self):
        self.holo.dbg_log('connman::wifiAgent::Cancel', 'enter_function',
                        0)
        self.holo.dbg_log('connman::wifiAgent::Cancel', 'exit_function',
                        0)
        return
...
class vpnAgent(dbus.service.Object):
    ...
    @dbus.service.method('net.connman.vpn.Agent', in_signature='',
                         out_signature='')
    def Release(self):
        self.holo.dbg_log('connman::vpnAgent::Release', 'enter_function',
                        0)
        self.holo.dbg_log('connman::vpnAgent::Release',
                        'exit_function (CANCELED)', 0)
    ...
    def RequestInput(self, path, fields):
        ...
        try:
            ...
            self.holo.dbg_log('connman::vpnAgent::RequestInput', 'enter_function', 0)
            ...
            self.holo.dbg_log('connman::vpnAgent::RequestInput', 'exit_function', 0)

            self.busy()
            return response

        except Exception, e:
            self.holo.dbg_log('connman::vpnAgent::RequestInput', 'ERROR: (' + repr(e) + ')', 4)

    @dbus.service.method('net.connman.vpn.Agent', in_signature='os', out_signature='')
    def ReportError(self, path, error):
        self.holo.dbg_log('connman::vpnAgent::ReportError',
                        'enter_function', 0)
        self.holo.dbg_log('connman::vpnAgent::ReportError',
                        'exit_function', 0)
        return
    ...
    def Cancel(self):
        self.holo.dbg_log('connman::vpnAgent::Cancel', 'enter_function', 0)
        self.holo.dbg_log('connman::vpnAgent::Cancel', 'exit_function', 0)
